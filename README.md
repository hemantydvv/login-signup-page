This project is a complete User Authentication and Role Management System built using PHP, MySQL, and Bootstrap. It provides a secure workflow for user registration and login with features like password hashing (using PHP’s password_hash), server-side validation, and protected routes using PHP sessions.

The system supports two user roles: Admin and User, each gaining access to different dashboard content based on their permissions. Admin users receive elevated privilege visibility, while standard users are limited to basic access.
